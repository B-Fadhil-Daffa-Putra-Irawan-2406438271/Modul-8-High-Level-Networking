...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let transaction_service = MyTransactionService::default(); // <-- Inisialisasi service baru

    println!("Starting gRPC Server on {}", addr);

    Server::builder()
        .add_service(PaymentServiceServer::new(payment_service))
        .add_service(TransactionServiceServer::new(transaction_service)) // <-- Daftarkan ke server
        .serve(addr)
        .await?;
